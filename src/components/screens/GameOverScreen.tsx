...
export default function GameOverScreen() {
  const { teams, resetGame, startGame, setTeams, settings } = useGameStore();
  ...
  return (
    <div className="screen fade-in" style={{ alignItems: 'center', textAlign: 'center', gap: 28 }}>
      ...
      <div className="col" style={{ gap: 8 }}>
        <p className="label">
          {isTie ? '平手！It\'s a Tie!' : '勝利者 Winner'}
        </p>
        {!isTie && (
          <h1 className="title-xl bounce-in" style={{ color: winner.color }}>
            {winner.name}
          </h1>
        )}
        {isTie && (
        ...
        <div className="col" style={{ gap: 10 }}>
          {sorted.map((t, i) => (
            <div key={t.id} className="row-between" style={{
              ...
              <span style={{ fontFamily: 'Fredoka One, cursive', fontSize: '1.6rem', color: t.color }}>
                {t.score}
              </span>
            </div>
          ))}
      ...
      <div className="col" style={{ width: '100%', gap: 12, marginBottom: 8 }}>
        <button className="btn btn-primary btn-lg btn-block" onClick={playAgain}>
          🔄 再玩！Play Again!
        </button>
        <button className="btn btn-ghost btn-block" onClick={() => resetGame()}>
          🏠 返回主頁 Home
        </button>
      </div>
    </div>
